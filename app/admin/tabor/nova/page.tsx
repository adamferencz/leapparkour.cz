...
import { CAMP_SPORTS } from "@/lib/config";
import { createManualRegistration } from "../actions";

export const metadata: Metadata = {
  title: "Přidat přihlášku — tábor",
};
...
export default function NovaTaborPage() {
  return (
    <main className="mx-auto max-w-3xl px-5 py-8">
      ...
      <h1 className="mt-3 text-2xl font-bold text-navy">Přidat přihlášku na tábor</h1>
      ...
      <Card className="mt-6 p-5">
        <form action={createManualRegistration} className="grid gap-4 md:grid-cols-2">
          ...
          <label>
            ...
            <input name="father_name" className={inputClass()} />
          </label>
          <label>
            <span className="mb-1.5 block text-sm font-medium text-navy">
              Jméno a příjmení matky
            </span>
            <input name="mother_name" className={inputClass()} />
          </label>
          <label>
          ...
          <label>
            <span className="mb-1.5 block text-sm font-medium text-navy">
              Věk dítěte na táboře
            </span>
            <input name="child_age" type="number" min={1} max={25} className={inputClass()} />
          </label>
          <label>
            <span className="mb-1.5 block text-sm font-medium text-navy">
              Datum narození dítěte *
            </span>
            <input name="child_birthdate" type="date" required className={inputClass()} />
          </label>
          <label>
            <span className="mb-1.5 block text-sm font-medium text-navy">
              Telefon na matku
            </span>
            <input name="phone_mother" type="tel" className={inputClass()} />
          </label>
          <label>
            <span className="mb-1.5 block text-sm font-medium text-navy">
              Telefon na otce
            </span>
            <input name="phone_father" type="tel" className={inputClass()} />
          </label>
          ...
          <fieldset className="md:col-span-2">
            <legend className="mb-1.5 text-sm font-medium text-navy">Sporty a aktivity</legend>
            <div className="grid gap-2 sm:grid-cols-2">
              {CAMP_SPORTS.map((sport) => (
                <label
                  key={sport}
                  className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm"
                >
                  <input
                    type="checkbox"
                    name="sports"
                  ...
                  />
                  <span className="font-medium text-navy">{sport}</span>
                </label>
              ))}
            </div>
          </fieldset>
          <label className="md:col-span-2">
            <span className="mb-1.5 block text-sm font-medium text-navy">Jiné</span>
            <input name="sports_other" className={inputClass()} />
          </label>
          <label className="md:col-span-2">
            <span className="mb-1.5 block text-sm font-medium text-navy">
              S kým chce být v chatce/pokoji
            </span>
            <input name="roommates" className={inputClass()} />
          </label>
          <label className="md:col-span-2">
            <span className="mb-1.5 block text-sm font-medium text-navy">
              Zdravotní omezení
