...
/** „4. 7. – 11. 7. 2027" → ["4. 7.", "11. 7. 2027"] */
const dateParts = CAMP.dates.split("–").map((part) => part.trim());
const arrivalDate = `${dateParts[0] ?? ""} ${CAMP.year}`.trim();
const departureDate = dateParts[1] ?? CAMP.dates;
    ...
    <div className="container-site py-16 md:py-24">
        ...
        <div className="rounded-3xl bg-slate-100 p-8">
          ...
          <p className="mt-4">
            <strong className="text-navy">
              Příjezd na tábor bude {arrivalDate} od 16:00 do 17:00
            </strong>{" "}
            (prosíme nejezděte dříve kvůli probíhajícím přípravám).
          </p>
          <p className="mt-3">
            <strong className="text-navy">
              Odjezd z tábora bude {departureDate} od 10:00 do 11:00.
            </strong>{" "}
            Děti dostanou obědový balíček.
          </p>
        </div>
      </div>
      ...
      <section className="mt-16 md:mt-24">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="mt-4 inline-block rounded-full bg-brand px-6 py-3 font-semibold text-white transition-colors hover:bg-brand-dark"
        >
      ...
      <section className="mt-16 md:mt-24">
        ...
        <ul className="mt-8 space-y-4">
          {CANCELLATION_TERMS.map((term) => (
            <li key={term} className="rounded-2xl bg-slate-100 px-6 py-5 leading-relaxed">
              {term}
            </li>
          ))}
        </ul>
